...
app.config['SECRET_KEY'] = 'yusran123'
...
@app.route('/', methods=['GET']) # =============================================================================Halaman Utama
def index():
...
@app.route('/login', methods=['POST']) # =============================================================================Halaman Login
def login():
    name = request.form.get('name', '').strip() # mengambil nama dari form
    nim = request.form.get('nim', '').strip()
    if not name or not nim:
        flash('Nama dan NIM harus diisi.', 'error')  # validasi sederhana bahwa nama dan nim tidak boleh kosong
...
@app.route('/logout') # =============================================================================Fungsi Logout
def logout():
    session.clear()
...
def dashboard():
    if not is_logged_in():
        ...
        return redirect(url_for('index'))
    rooms = Room.query.order_by(Room.name).all() # menampilkan semua ruangan yang ada di database
    return render_template('dashboard.html', rooms=rooms, user=session) 

@app.route('/book/<int:room_id>', methods=['GET']) # =============================================================================Halaman Booking
def book_form(room_id):
    if not is_logged_in():
        flash('Silakan login terlebih dahulu.', 'error')
        return redirect(url_for('index'))
    room = Room.query.get_or_404(room_id) # mendapatkan data ruangan berdasarkan id
    return render_template('booking.html', room=room, user=session) 

@app.route('/book_submit', methods=['POST']) # =============================================================================Fungsi Submit Booking
def book_submit():
    if not is_logged_in():
        flash('Silakan login terlebih dahulu.', 'error')
        return redirect(url_for('index'))
    ...
    db.session.flush()  # untuk dapatkan booking.id sebelum commit
    # update status room
